...
if(expr1,expr2,expr3)  -- select if(username='a',"男","女") from t_demo 双分支
select (case 字段 when 值 then 值 else 值 end) from 表 -- 相当于if else
...
min();
avg();
group_concat() -- 对分组的结果中的某个字段进行字符串连接。结果会用","连接
...
> 下面的东西基本不用看，都是很基础的东西，需要用到的时候上来看看就可以了
...
-- 导出
1. 导出一张表
　　mysqldump -u用户名 -p密码 库名 表名 > 文件名(D:/dhy.sql)
2. 导出多张表
　　mysqldump -u用户名 -p密码 库名 表1 表2 表3 > 文件名(D:/dhy.sql)
3. 导出所有表
　　mysqldump -u用户名 -p密码 库名 > 文件名(D:/dhy.sql)
4. 导出一个库 
　　mysqldump -u用户名 -p密码 -B 库名 > 文件名(D:/dhy.sql)
